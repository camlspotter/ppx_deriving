...
let conv s = Exp.(ident @@ lid  @@ "Conv." ^ s)
...
*)
    ...
    | [%type: int]    -> conv "int"
    | [%type: int32]     | [%type: Int32.t] -> conv "int32"
    | [%type: int64]     | [%type: Int64.t] -> conv "int64"
    | [%type: nativeint] | [%type: Nativeint.t] -> conv "nativeint"
    | [%type: float]  -> conv "float"
    | [%type: bool]   -> conv "bool"
    | [%type: char]   -> conv "char"
    | [%type: string] -> conv "string"
    | [%type: bytes]  -> conv "bytes"
    | [%type: [%t? typ] ref]   ->
        [%expr [%e conv "ref"] [%e expr_of_typ typ] ]
    | [%type: [%t? typ] list]  -> 
        [%expr [%e conv "list"] [%e expr_of_typ typ] ]
    | [%type: [%t? typ] array] ->
        [%expr [%e conv "array"] [%e expr_of_typ typ] ]
    | [%type: [%t? typ] option] ->
        [%expr [%e conv "option"] [%e expr_of_typ typ] ]
    | { ptyp_desc = Ptyp_arrow (l, t1, t2) } ->
        [%expr [% conv "arrow"] l [%e expr_of_typ t1] [%e expr_of_typ t2]]
    | { ptyp_desc = Ptyp_constr ({ txt = lid }, args) } ->
        let args_pp = List.map (fun typ -> [%expr [%e expr_of_typ typ]]) args in
        app (Exp.ident (mknoloc (Ppx_deriving.mangle_lid (`Prefix "conv_of") lid)))
          args_pp
    | { ptyp_desc = Ptyp_tuple typs } ->
        let args = List.mapi (fun i typ -> app (expr_of_typ typ) [evar (argn i)]) typs in
        [%expr
            fun [%p ptuple (List.mapi (fun i _ -> pvar (argn i)) typs)] ->
              [%e conv "tuple"] [%e list args]
        ]
    ...
    | Ptype_variant constrs, _ ->
        let cases =
          constrs |> List.map (fun { pcd_name = { txt = name' }; pcd_args } ->
            ...
            let result =
              [%expr [%e conv "variant"] [%e str constr_name] 
                  [%e list args]
              ]
            in
...
let sig_of_type ~options ~path type_decl =
  ...
  [ Sig.value (Val.mk (mknoloc (Ppx_deriving.mangle_type_decl (`Prefix "conv_of") type_decl)) (polymorphize [%type: [% typ] -> conv]))
  ]
(*
  let polymorphize = Ppx_deriving.poly_arrow_of_type_decl
        (fun var -> [%type: Format.formatter -> [%t var] -> unit]) type_decl in
