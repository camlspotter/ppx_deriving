...
INSTALL_TARGETS=`grep -E '^\s+' ppx_deriving.install | sed -r -e 's/^\s+"\??([^"]+)".*/\1/'`
...
DIR=`ocamlfind query ppx_deriving | sed -e 's/\\r$//'`
echo $DIR
mv $DIR/ppx_deriving_main.native $DIR/ppx_deriving.exe
